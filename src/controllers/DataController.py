...
class DataController(BaseController):
    ...
    def validate_upload_file(self, file: UploadFile) -> tuple[bool, str]:
        result = True, ResponseConfig.FILE_PASSED_VALIDATION.value
        if file.content_type not in self.app_settings.FILE_ALLOWED_TYPES:
            result = False, ResponseConfig.FILE_TYPE_NOT_SUPPORTED.value
        elif file.size > self.app_settings.FILE_MAX_SIZE * self.file_scale:
            result = False, ResponseConfig.FILE_EXCEEDED_MAX_SIZE.value
        return result
    ...
    async def write_uploaded_file(
        ...
        result = True, ResponseConfig.FILE_UPLOAD_SUCCEEDED.value
        try:
            ...
            result = False, ResponseConfig.FILE_UPLOAD_FAILED.value
        return result
